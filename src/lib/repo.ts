...
export async function addAppointment(p: ParsedAppointment, tz: string): Promise<{ appointment: Appointment; reminder: Reminder }> {
...
export function latestReminder(row: AppointmentRow): Reminder | null {
...
export async function updateAppointment(row: AppointmentRow, patch: AppointmentPatch): Promise<void> {
  ...
  const cur = latestReminder(row)
  if (cur && cur.status !== 'sent') {
    await db.reminderUpdate(cur.id, { remind_at, status: 'pending', attempts: 0, next_attempt_at: null, last_error: null })
  } else if (remind > now) {
    await db.reminderInsert({ appointment_id: row.id, remind_at })
  }
}
