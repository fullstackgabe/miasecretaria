...
defineProps<{ row: AppointmentRow; tz: string; busy: boolean }>()
const emit = defineEmits<{ close: []; confirm: [] }>()
</script>

<template>
  <ModalSheet @close="emit('close')">
    <h2 class="mb-2 text-lg font-extrabold text-ink">Desmarcar compromisso?</h2>
    ...
    <div class="mt-[18px] flex gap-2.5">
      ...
      <button type="button" class="btn-danger flex items-center justify-center" :disabled="busy" @click="emit('confirm')">
        ...
        <template v-else>Desmarcar</template>
      </button>
    </div>
  </ModalSheet>
</template>
